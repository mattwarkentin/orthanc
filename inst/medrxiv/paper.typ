...
#set page(
  header: context {
    if counter(page).get().first() > 1 {
      align(right)[
        *orthanc: An R Interface to Orthanc DICOM Servers*
      ]
    }
  }
)
...
#show: arkheion.with(
  ...
  authors: (
    (name: "Matthew T. Warkentin", email: text(link("mailto:matthew.warkentin@ucalgary.ca"), fill: blue), affiliation: "Department of Oncology, Cumming School of Medicine, University of Calgary, Calgary, AB, Canada", orcid: "0000-0001-8730-3511"),
  ),
  abstract: text()[Medical imaging data are increasingly used in biomedical research, artificial intelligence (AI), and precision medicine, creating a growing need for scalable and reproducible tools for accessing and managing large DICOM archives. Orthanc is an open-source DICOM server that provides a powerful REST API for storing, querying, and retrieving medical imaging data; however, direct interaction with this API requires low-level HTTP programming and familiarity with complex DICOM resource structures. This paper presents `orthanc`, an R package that provides a high-level, idiomatic interface to the Orthanc REST API. The package represents DICOM resources as structured R objects, simplifies querying and retrieval of imaging data, abstracts HTTP communication, and supports asynchronous operations for high-throughput workflows. By integrating Orthanc directly into the R ecosystem, orthanc enables reproducible imaging workflows and facilitates the analysis of medical imaging data alongside clinical, statistical, and machine learning applications.],
  keywords: ("R", "Orthanc", "Medical physics", "Medical imaging"),
  date: "July 28, 2026",
)
...
+ Response parsing and data management
...
The `mirai` framework is built on top of `nanonext` @nanonext, an R interface to the Nanomsg Next Generation (NNG) messaging library. This architecture provides a lightweight, high-performance, message-passing infrastructure that supports scalable asynchronous and parallel computation with minimal overhead. By leveraging `mirai` and its NNG-based backend, `orthanc` offers a robust and efficient asynchronous client model that scales from local multicore environments to distributed systems, while maintaining a simple and idiomatic R interface. 
...
In summary, `orthanc` fills a critical gap in the R ecosystem by providing a robust, high-level interface to Orthanc DICOM servers. As medical imaging continues to drive innovation in AI and quantitative research, accessible and reproducible tools for managing DICOM data are essential. By simplifying interaction with Orthanc and embedding imaging workflows directly within R, `orthanc` supports the growing community of researchers working at the intersection of imaging, statistics, and data science. To facilitate adoption and reproducible research, the `orthanc` package is openly available through the Comprehensive R Archive Network (https://doi.org/10.32614/CRAN.package.orthanc) and the project's GitHub repository (https://github.com/mattwarkentin/orthanc). These resources provide installation instructions, documentation, examples, and mechanisms for community feedback and future development.
